...
if (isset($_GET['success']) && $_GET['success'] == '1') {
    $success_action = $_GET['action'] ?? 'updated';
}

// AJAX: Restore stock when unchecking an item
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['order_id'])) {
    ...
    $fetch = $conn->prepare("SELECT items, total_amount, table_number FROM orders WHERE id = ? AND restaurant_id = ?");
    $fetch->bind_param("ii", $order_id, $restaurant_id);
    $fetch->execute();
    $cur = $fetch->get_result()->fetch_assoc();
    $fetch->close();

    if (!$cur) {
        $error_msg = "Order not found.";
    } else {
        $current_items = json_decode($cur['items'], true) ?? [];
        $current_qty = [];
        ...
        $old_data = json_encode([
            ...
            'table_number' => $cur['table_number']
        ]);
        $time = nepali_date_time();
        $remark = isset($_POST['delete_order']) ? 'Order deleted by staff' : 'Order edited by staff';

        $audit = $conn->prepare("INSERT INTO old_order (order_id, restaurant_id, changed_by, change_time, old_data, remarks) VALUES (?, ?, ?, ?, ?, ?)");
        $audit->bind_param("iiisss", $order_id, $restaurant_id, $staff_id, $time, $old_data, $remark);
        $audit->execute();
        $audit->close();

        try {
            if (isset($_POST['delete_order']) && $_POST['delete_order'] === '1') {
                // Delete + full stock restore
                foreach ($current_items as $item) {
                    ...
                    $upd->bind_param("iis", $qty, $restaurant_id, $name);
                    $upd->execute();
                    $upd->close();
                }
                ...
                header("Location: edit_orders.php?success=1&action=deleted");
                exit;
            } else {
                // Edit order
                ...
                foreach ($selected_items as $item_id_str) {
                    ...
                    if ($diff != 0) {
                        ...
                        $exists_check->bind_param("is", $restaurant_id, $name);
                        ...
                        if ($exists) {
                            if ($diff > 0) {
                                ...
                                if ($stock_row['quantity'] < $diff) {
                                    throw new Exception("Not enough stock for <strong>$name</strong>.");
                                }
                            }
                            $op = $diff > 0 ? '-' : '+';
                            $abs_diff = abs($diff);
                            $adjust = $conn->prepare("UPDATE stock_inventory SET quantity = quantity $op ? WHERE restaurant_id = ? AND stock_name = ?");
                            $adjust->bind_param("iis", $abs_diff, $restaurant_id, $name);
                            $adjust->execute();
                            $adjust->close();
                        }
                    }
                ...
                $updated_at = nepali_date_time();

                $upd = $conn->prepare("UPDATE orders SET table_number = ?, items = ?, total_amount = ?, updated_at = ? WHERE id = ? AND restaurant_id = ?");
                $upd->bind_param("ssdssi", $table_identifier, $items_json, $total, $updated_at, $order_id, $restaurant_id);
                $upd->execute();
                $upd->close();

                $conn->commit();
                header("Location: edit_orders.php?success=1&action=updated");
                exit;
            }
        } catch (Exception $e) {
            $conn->rollback();
...
// Fetch orders
$stmt = $conn->prepare("SELECT o.*, u.username AS ordered_by_name FROM orders o LEFT JOIN users u ON o.order_by = u.id WHERE o.restaurant_id = ? AND o.status = 'preparing' ORDER BY o.id DESC");
...
<head>
    ...
    <style>
        ...
        .order-card-wrapper { margin-bottom: 2rem; }
...
<div class="toast-container">
    <div id="successToast" class="toast align-items-center text-bg-success border-0 shadow-lg" role="alert" aria-live="assertive" aria-atomic="true" style="display: none;">
        <div class="d-flex">
...
<div class="container py-4 py-md-5">
    ...
    <div class="accordion" id="ordersAccordion">
        ...
        <div class="accordion-item order-card-wrapper">
            <h2 class="accordion-header" id="heading<?= $order['id'] ?>">
                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?= $order['id'] ?>">
                    Order #<?= $order['id'] ?> — <?= htmlspecialchars($order['table_number']) ?> — Rs <?= number_format($order['total_amount'], 2) ?>
                </button>
            </h2>
            <div id="collapse<?= $order['id'] ?>" class="accordion-collapse collapse" data-bs-parent="#ordersAccordion">
                <div class="accordion-body">
                    <form method="POST" id="editForm_<?= $order['id'] ?>">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-bold">Table / Name / Phone</label>
                            <input type="text" name="table_identifier" class="form-control" value="<?= htmlspecialchars($order['table_number']) ?>" required>
                        </div>
                        ...
                        <div class="text-center mt-4">
                            <button type="button" class="btn btn-primary me-2" id="updateBtn_<?= $order['id'] ?>">Update Order</button>
...
<script>
    ...
    document.querySelectorAll('[id^="updateBtn_"]').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (!table) {
                alert('Please enter Table / Name / Phone.');
                return;
            }
    ...
    document.getElementById('confirmUpdateBtn').addEventListener('click', () => {
        ...
        fetch('edit_orders.php', {
            ...
            window.location.href = 'edit_orders.php?success=1&action=updated';
        }).catch(() => {
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', () => {
        ...
        fetch('edit_orders.php', {
            ...
            window.location.href = 'edit_orders.php?success=1&action=deleted';
        }).catch(() => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        <?php if (!empty($success_action)): ?>
            const toastEl = document.getElementById('successToast');
            const toastMessage = document.getElementById('toastMessage');
            if (toastEl && toastMessage) {
                toastMessage.textContent = "Order successfully <?= $success_action === 'deleted' ? 'deleted' : 'updated' ?>!";
