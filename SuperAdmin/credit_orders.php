<?php
// SuperAdmin/credit_orders.php - Current Orders with "Mark Credit" option
session_start();
...
<!-- Mark Credit Modal - Final Clean Version -->
<div class="modal fade" id="markCreditModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-danger text-white">
                <h5 class="modal-title">Mark Order as Credit (Udhar)</h5>
            ...
            <div class="modal-body">
                ...
                <div class="alert alert-info py-3 text-center mb-4">
                    ...
                    <span id="creditPreviousDue" class="fs-4 fw-bold ms-3">Rs. 0.00</span>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered">
                        <tbody>
                            <tr>
                                <td class="text-start fw-bold bg-light">Subtotal</td>
                            ...
                            <tr id="discountPreviewRow" style="display:none;">
                                ...
                                <td class="text-end text-danger" id="previewDiscount">0.00</td>
                            </tr>
                            <tbody id="previewChargesBody"></tbody>
                            <tr>
                                <td class="text-start fw-bold bg-light">Taxable Amount</td>
                                <td class="text-end fw-bold" id="previewTaxable">0.00</td>
                            </tr>
                            <tr id="vatRow" style="display:none;">
                                <td class="text-start">VAT (<span id="vatPercentDisplay">13</span>%)</td>
                                <td class="text-end" id="previewVat">0.00</td>
                            </tr>
                            <tr class="table-primary">
                                <td class="text-start fw-bold">Net Total</td>
                                <td class="text-end fw-bold" id="previewNetTotal">0.00</td>
                            </tr>
                            <tr class="table-warning">
                                <td class="text-start fw-bold">Total Amount Due</td>
                                <td class="text-end fw-bold fs-5 text-danger" id="totalAmountDue">0.00</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="mb-3 position-relative">
                    ...
                    <input type="text" id="creditCustomerSearch" class="form-control" placeholder="Type name or phone..." autocomplete="off">
                    <div id="creditCustomerDropdown" class="dropdown-menu w-100 shadow"></div>
                </div>

                <div id="newCustomerFields" style="display:none;">
                    <div class="mb-3">
                        <label class="form-label">Phone</label>
                        <input type="text" id="newCustomerPhone" class="form-control" placeholder="Optional">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Address</label>
                        <textarea id="newCustomerAddress" class="form-control" rows="2" placeholder="Optional"></textarea>
                    </div>
                </div>

                <div id="selectedCreditCustomer" class="alert alert-success py-2" style="display:none;">
                    <strong>Selected:</strong> <span id="selectedCreditCustomerName"></span>
                </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="number" id="creditDiscount" class="form-control" min="0" step="0.01" value="0.00">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-bold">Partial Payment Method</label>
                        <select id="creditPaymentMethod" class="form-select">
                ...
                <div class="mt-3">
                    <label class="form-label fw-bold">Amount Paid Today (Rs)</label>
                    <input type="number" id="creditPaidToday" class="form-control" min="0" step="0.01" value="0.00">
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="confirmMarkCredit" class="btn btn-primary btn-lg">Next →</button>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-danger">
            ...
            <div class="modal-body text-center py-4">
                <p><strong>Customer:</strong> <span id="confirmCustomerName" class="fw-bold"></span></p>
                <p><strong>Total Amount Due:</strong> Rs. <span id="confirmNetTotal" class="fw-bold fs-4 text-danger">0.00</span></p>
                <p><strong>Paid Today:</strong> Rs. <span id="confirmPaidToday" class="fw-bold">0.00</span></p>
                <hr>
                <h5 class="text-danger fw-bold">No bill will be printed!</h5>
                <p>Are you sure you want to mark this order as Credit/Udhar?</p>
            </div>
            <div class="modal-footer justify-content-center">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <button type="button" id="finalConfirmCredit" class="btn btn-danger btn-lg px-5">Yes, Mark as Credit</button>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    .then(data => {
        if (data.success) {
            orderData.charges = data.charges;
            orderData.vatPercent = data.vat_percent || 0;
            document.getElementById('vatPercentDisplay').textContent = orderData.vatPercent;
        }
    });
});
...
document.querySelectorAll('.mark-credit-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        const rawTotal = parseFloat(this.dataset.total) || 0;

        document.getElementById('creditOrderId').value = orderId;

        orderData.subtotal = rawTotal;
        document.getElementById('previewSubtotal').textContent = rawTotal.toFixed(2);
