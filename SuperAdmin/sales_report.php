...
// 2. ITEM-WISE SALES WITH CURRENT PRICING
...
// === PURCHASE DATA (regular + general bills) ===
...
// === CREDIT SUMMARY (overall from customers table) ===
$credit_summary = ['total_consumed' => 0.00, 'total_paid' => 0.00, 'remaining_due' => 0.00];

$credit_query = "SELECT 
                    COALESCE(SUM(total_consumed), 0) AS consumed,
                    COALESCE(SUM(total_paid), 0) AS paid,
                    COALESCE(SUM(remaining_due), 0) AS due 
                 FROM customers 
                 WHERE restaurant_id = ?";

$credit_stmt = $conn->prepare($credit_query);
if ($credit_stmt) {
    $credit_stmt->bind_param("i", $restaurant_id);
    $credit_stmt->execute();
    ...
    $credit_summary['remaining_due']  = (float)$credit_row['due'];
...
<div class="container-fluid mt-4">
        ...
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                ...
                <div class="btn-group">
                    ...
                    <a href="view_branch.php" class="btn btn-light btn-sm">
                ...
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-md-2 filter-group" id="weekFilter" style="display: <?= ($period === 'week' && $nepali_month) ? 'block' : 'none' ?>;">
                        ...
                        <select name="week_num" class="form-select">
                            <?php for ($w = 1; $w <= 5; $w++): ?>
                                <option value="<?= $w ?>" <?= $week_num == $w ? 'selected' : '' ?>>Week <?= $w ?></option>
                            <?php endfor; ?>
                        </select>
                    </div>
    ...
    <div class="row g-4 mb-5">
        ...
        <div class="col-xl-3 col-lg-6 col-md-6">
            <div class="card stat-card h-100 text-center <?= $net_profit >= 0 ? 'bg-success' : 'bg-danger' ?> text-white">
                <div class="card-body py-4">
        ...
        <div class="col-xl-3 col-lg-6 col-md-6">
            <div class="card stat-card h-100 text-center bg-info text-white">
                <div class="card-body py-4">
                    ...
                    <p class="mb-2">Unique Items Sold</p>
                </div>
            </div>
        </div>
    </div>
