<?php
// SuperAdmin/bill.php – FULL PHP BLOCK ONLY – 100% WORKING
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_mark_paid'])) {
    ...
    $stmt_phone = $conn->prepare("SELECT phone_number FROM restaurants WHERE id = ?");
    $stmt_phone->bind_param('i', $restaurant_id);
    $stmt_phone->execute();
    $phone_row = $stmt_phone->get_result()->fetch_assoc();
    $stmt_phone->close();
    $admin_phone = $phone_row['phone_number'] ?? 'N/A';
    ...
    $bill_details = [
        ...
        'printed_by' => $_SESSION['username'] ?? 'SuperAdmin',
...
// === FETCH ORDER ===
$stmt = $conn->prepare("
    SELECT o.*, r.name as restaurant_name, r.address, r.pan_number, r.phone_number as admin_phone
    FROM orders o 
...
$printed_by = $bill_details['printed_by'] ?? ($_SESSION['username'] ?? 'SuperAdmin');
    ...
    <div class="row">
        <div class="col-md-6 mx-auto">
            <div id="billFormContainer">
                <?php if (!$is_paid): ?>
                <form id="billForm" class="p-3 border rounded bg-light"> 
                    <div class="mb-2">
...
document.getElementById('markPaidBtn')?.addEventListener('click', function() {
    ...
    // Find or create the error message (only once)
    let errorMsg = paymentMethodSelect.parentNode.querySelector('.invalid-feedback');
    if (!errorMsg) {
        errorMsg = document.createElement('div');
        errorMsg.className = 'invalid-feedback';
        errorMsg.textContent = 'Please select a payment method.';
        paymentMethodSelect.parentNode.appendChild(errorMsg);
    }

    // Validate
    if (!paymentMethodSelect.value) {
        paymentMethodSelect.classList.add('is-invalid');
        // No need to manually show — Bootstrap handles it via .is-invalid
        return;
    }

    // If valid, Bootstrap automatically hides it when .is-invalid is removed
    ...
    .then(data => {
        if (data.success) {
            ...
            document.getElementById('discountRow').style.display = bd.discount > 0 ? '' : 'none';
            ...
            if (document.getElementById('vatAmt')) {
                document.getElementById('vatAmt').innerText = parseFloat(bd.vat_amount).toFixed(2);
            }
