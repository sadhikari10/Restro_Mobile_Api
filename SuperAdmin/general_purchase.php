...
// SAFE RESTAURANT SESSION LOADER — NEVER FAILS
if (empty($_SESSION['current_restaurant_id'])) {
    die("Error: No restaurant selected. Please go back and select a restaurant.");
}

// Load restaurant name if missing (this is the key fix)
if (empty($_SESSION['current_restaurant_name'])) {
    $stmt = $conn->prepare("SELECT name FROM restaurants WHERE id = ? LIMIT 1");
    $stmt->bind_param("i", $_SESSION['current_restaurant_id']);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($row = $result->fetch_assoc()) {
        $_SESSION['current_restaurant_name'] = $row['name'];
    } else {
        $_SESSION['current_restaurant_name'] = "Unknown Restaurant";
    }
    $stmt->close();
}

$restaurant_id   = (int)$_SESSION['current_restaurant_id'];
$restaurant_name = $_SESSION['current_restaurant_name'];  // Always exists now
...
// Get full Nepali date + time like: 2082-04-27 14:30:22
$nepali_datetime_full = nepali_date_time();  // Your existing function returns "2082-04-27 14:30:22"
$inserted_at_bs = $nepali_datetime_full;     // This is what we'll save;
...
<head>
    <meta charset="utf-8">
    <title>General Bill Entry</title>
...
<div class="container mt-4">
    ...
    <?php if ($message): ?>
        <div id="msgBox" class="mb-4"><?= $message ?></div>
    <?php endif; ?>

    <div class="alert alert-info small mb-4">
        Restaurant: <strong><?= htmlspecialchars($restaurant_name) ?></strong>
    </div>

    <div class="mt-3">
        <a href="view_branch.php" class="btn btn-dark">
            Back
        </a>
    </div>
